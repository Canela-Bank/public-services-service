        ...
        try {
            ...
            if(connAccount.getResponseCode() == HttpURLConnection.HTTP_OK){
                ...
                if(salesService > Integer.parseInt(jsonAccount.get("balance").toString())){
                    ...
                    URL updateAccount = new URL("http://10.1.0.0:3001/graphql?query=mutation%7B%0A%20%20createAccount%20(id%3A%22" + request.getSavingsId() + "%22%2C%20balance%3A%20" + newBalance + "%2C%20user_id%3A%20%22" + request.getUserId() + "%22)%7B%0A%20%20%20%20id%0A%20%20%20%20balance%0A%20%20%20%20user_id%0A%20%20%7D%0A%7D%0A");
